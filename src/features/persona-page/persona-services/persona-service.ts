...
import { PersonasContainer } from "@/features/common/services/cosmos";
...
export const FindPersonaByID = async (
  ...
  try {
    ...
    const { resources } = await PersonasContainer()
...
export const CreatePersona = async (
  ...
  try {
    ...
    const { resource } = await PersonasContainer().items.create<PersonaModel>(
      modelToSave
    );
...
export const DeletePersona = async (
  ...
  try {
    ...
    if (personaResponse.status === "OK") {
      const { resource: deletedPersona } = await PersonasContainer()
...
export const UpsertPersona = async (
  ...
  try {
    ...
    if (personaResponse.status === "OK") {
      ...
      const { resource } = await PersonasContainer().items.upsert<PersonaModel>(
        modelToUpdate
      );
